...
export default function LoginForm() {
  ...
  async function handleGoogleCallback(response: any) {
    ...
    try {
      // Step 1 — send id_token to backend
      ...
      if (data.requires_setup) {
        // New user — redirect to setup page with setup_token
        window.location.href = `/auth/setup?token=${data.setup_token}`;
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div>
            <div className="flex justify-between mb-2">
              ...
              <Link href="/auth/forgot-password" className="text-indigo-400 text-sm">
                Forgot?
              </Link>
            </div>
            <div className="relative">
              <input
                ...
                placeholder="••••••••"
                className="w-full rounded-lg bg-slate-950 border border-slate-700 px-4 py-3 outline-none focus:border-indigo-500 pr-12"
              />
              <button
                type="button"
